...
const TrainerReviewsPage = () => {
      ...
      try {
        ...
        const trainersWithImages = data.data
          ...
          .map((trainer: Trainer) => ({
            ...
            // Ensure pricingPlans exists and pricingPlan is set to first item
            pricingPlans: trainer.pricingPlans || [trainer.pricingPlan].filter(Boolean),
            pricingPlan: trainer.pricingPlan || (trainer.pricingPlans?.[0] || 'N/A')
          }));
  ...
  const allPricingPlans = [...new Set(trainers.flatMap(trainer => trainer.pricingPlans || [trainer.pricingPlan].filter(Boolean)))];
  ...
  useEffect(() => {
    ...
    // Filter by pricing plan (check both pricingPlan and pricingPlans)
    if (selectedPricingPlan) {
      filtered = filtered.filter(trainer =>
        trainer.pricingPlans?.includes(selectedPricingPlan) || 
        trainer.pricingPlan === selectedPricingPlan
      );
    }
  ...
  const handleDeleteReview = async (reviewId: string) => {
    ...
    try {
      ...
      if (selectedTrainer) {
        const updatedReviews = selectedTrainer.reviews.filter(r => r._id !== reviewId);
        setSelectedTrainer({ ...selectedTrainer, reviews: updatedReviews });
      }
      
      setTrainers(prev => prev.map(tr =>
        tr.reviews.some(r => r._id === reviewId)
          ? { ...tr, reviews: tr.reviews.filter(r => r._id !== reviewId) }
          : tr
      ));
  ...
  const handleEditReview = async () => {
    if (!editReview || !editReview._id) return;
    setEditLoading(true);
    setActionMessage(null);
    try {
      ...
      setTrainers(prev => prev.map(tr =>
        tr.reviews.some(r => r._id === editReview._id)
          ? { ...tr, reviews: tr.reviews.map(r => r._id === editReview._id ? { ...r, comments: editComments, rating: editRating } : r) }
          : tr
      ));
          ...
          <div className="fixed inset-0 bg-black bg-opacity-70 flex justify-center items-center z-50 px-4">
            <div className="bg-white text-black p-6 rounded-2xl max-w-5xl w-full relative max-h-[90vh] shadow-2xl grid grid-cols-1 md:grid-cols-2 gap-6 overflow-hidden">
              ...
              <div>
                ...
                <div className="mt-2">
                  <p className="text-sm text-gray-700 font-semibold">💲 Pricing Plans:</p>
                  <ul className="list-disc list-inside text-sm">
                    {(selectedTrainer.pricingPlans || [selectedTrainer.pricingPlan].filter(Boolean)).map((plan, idx) => (
                      <li key={idx} className={idx === 0 ? "text-red-600" : ""}>
                        {plan}
                      </li>
                    ))}
                  </ul>
                          ...
                          {userEmail && userEmail === review.memberEmail && (
                            <div className="absolute top-2 right-2 flex gap-2">
                              <button
                                ...
                                disabled={editLoading || deleteLoading === review._id}
                              >
                                {editLoading && editReview?._id === review._id ? 'Saving...' : 'Edit'}
                              </button>
                              <button
                                ...
                                disabled={deleteLoading === review._id || editLoading}
                              >
